...
public class CheckLogin extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String UID = new model.API().CheckLogin(user, pass);
        if (UID != null) {
            HttpSession session = request.getSession();
            session.setAttribute("id", UID);
            if (request.getParameter("check") != null) {
                Cookie cookieIdUser = new Cookie("id", UID);
                cookieIdUser.setMaxAge(60 * 60 * 24);
                response.addCookie(cookieIdUser);
            }
            request.getRequestDispatcher("index.jsp").forward(request, response);
        } else {
            request.setAttribute("status", "login false");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Cookie cookieIdUser = new Cookie("id", null);
        response.addCookie(cookieIdUser);
